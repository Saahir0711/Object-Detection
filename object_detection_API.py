...
def ask_image():
    ...
    while True:
        ...
        if os.path.splittext(p)[1].lower() not in ALLOWED: print("Unsupported type")
...
def infer(path, img_bytes, tries = 8):
    mime, __ = mimetypes.guess_type(path)
    for i in range(tries):
        if mime and mime.startswith("image/"):
            r = requests.post(API, headers={"Authorization": f"Bearer {HF_API_KEY}", "Content-Type": mime}, data=img_bytes, timeout=60)
        else:
            r = requests.post(API, headers={"Authorization": f"Bearer {HF_API_KEY}"}, files = {"inputs": (os.path.basename(path), img_bytes, "application/octect-stream")}, timeout = 60)
        
        if r.status_code == 200:
            d = r.json()
            if isinstance(d, dict) and "error" in d: raise RuntimeError(d["error"])
            if not isinstance(d, list): raise RuntimeError(d["error"])
            return d
...
def main():
    ...
    except Exception as e: return print("X", e)
    ...
    print("\n Disclaimer: This is an AI model demo. Detections may not always be acurate or complete. Use it for fun and learning not for safety critical decision")
